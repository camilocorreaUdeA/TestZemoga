...
struct BMP {
    ...
    void horizontalFlip()
    {
        ...
        //std::reverse(data.begin()+75366, data.begin()+75843);
    }
    ...
    void verticalFlip()
    {
        ...
        //std::swap_ranges(data.begin(), data.begin()+rowSize, data.begin()+(numRows*rowSize));
    }

    void changeColors(uint8_t Bb, uint8_t Gb, uint8_t Rb, uint8_t Bw, uint8_t Gw, uint8_t Rw)
    {
        for (int32_t y = 0; y < bmp_info_header.height ; ++y) {
            for (int32_t x = 0; x < bmp_info_header.width; ++x) {
                ...
                if(data[channels * (y * bmp_info_header.width + x) + 0] < 230)
                {
